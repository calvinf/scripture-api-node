...
import { BibleBookParams, BibleBooksParams, BiblesParams, CommonFetchParams, SearchParams } from './types';
...
class ScriptureApi {
     ...
     */
    async getBibleBooks(bibleId: string, params: BibleBooksParams = {}): Promise<any> {
        try {
            const results = await Fetch.bibleBooks(this.#apiKey, bibleId, params);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get bible books: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBibleBook(bibleId: string, bookId: string, params: BibleBookParams = {}): Promise<any> {
        try {
            const results = await Fetch.bibleBook(this.#apiKey, bibleId, bookId, params);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get bible book: ${e.message}\n${e.stackTrace}`);
        }
    }
    ...
    async getBibleBookChapters(bibleId: string, bookId: string): Promise<any> {
        try {
            const results = await Fetch.bibleBookChapters(this.#apiKey, bibleId, bookId);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get bible book chapters: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBibleBookChapter(bibleId: string, chapterId: string, params: CommonFetchParams = {}): Promise<any> {
        try {
            const results = await Fetch.bibleBookChapter(this.#apiKey, bibleId, chapterId, params);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get bible book chapter: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBiblePassage(bibleId: string, passageId: string, params: CommonFetchParams = {}): Promise<any> {
        try {
            const results = await Fetch.biblePassage(this.#apiKey, bibleId, passageId, params);
    ...
    async search(bibleId: string, params: SearchParams = {}): Promise<any> {
        try {
            ...
            throw new Error(`Failed to get search results: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBibleBookSections(bibleId: string, bookId: string, params: CommonFetchParams = {}): Promise<any> {
        try {
            const results = await Fetch.bibleBookSections(this.#apiKey, bibleId, bookId, params);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get Bible book sections: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBibleChapterSections(bibleId: string, chapterId: string): Promise<any> {
        try {
            const results = await Fetch.bibleChapterSections(this.#apiKey, bibleId, chapterId);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get Bible chapter sections: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     */
    async getBibleSections(bibleId: string, sectionId: string, params: CommonFetchParams = {}): Promise<any> {
        try {
            const results = await Fetch.bibleSection(this.#apiKey, bibleId, sectionId, params);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get Bible sections: ${e.message}\n${e.stackTrace}`);
        }
    }
     ...
     * @returns {object} returns a promise
     */
    async getBibleChapterVerses(bibleId: string, chapterId: string): Promise<any> {
        try {
            const results = await Fetch.bibleChapterVerses(this.#apiKey, bibleId, chapterId);

            return results.json();
        } catch (e) {
            throw new Error(`Failed to get Bible chapter verses: ${e.message}\n${e.stackTrace}`);
        }
    }
